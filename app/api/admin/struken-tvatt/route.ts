...
export async function POST(request: NextRequest) {
  ...
  const { name, price, category, discountPercent, icon, inputDisabled, inputPlaceholder, unit, minUnits, maxUnits, minQty, rutEligible } = await request.json();
  ...
  const doc = {
    ...
    // Defaults to eligible when the caller says nothing, matching every item
    // that existed before RUT could be turned off.
    rutEligible:     normalizeRutEligible(rutEligible),
    ...pricing,
  };
